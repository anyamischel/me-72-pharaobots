...
const int INTAKE_SPEED1 = 128; // Speed we want for second switch position
const int INTAKE_SPEED2 = 255; // Speed we want for third switch position
////
...
void driveMotorCmd(RoboClaw &rc, uint8_t chan, int cmd) { // cmd ∈ [-127,127] driveMotorCmd(roboclaw1, M1, rightCmd)
  cmd = constrain(cmd, -127, 127); // ensures that the inputs to the roboclaw are within the apporpriate bounds
  uint8_t sp = (uint8_t)abs(cmd); // sp = speed
...
const int stopCmd = 0; // initializing a variable that we can later use when we want no movement
...
void setup() {
  ...
  // open up communication for iBus which is on pin 17 (RX2) => Serial2
  ...
  // Stop motors just in case so we don't die
